...
export default function AutomaticClassification(props) {
  const {
    id: idProp,
    inspection,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = inspectionRecord
      ? { ...initialValues, ...inspectionRecord }
      : initialValues;
    setImageS3Url(cleanValues.imageS3Url);
    setInspModelTS(cleanValues.inspModelTS);
    setInspManualTS(cleanValues.inspManualTS);
    setInspModelClass(cleanValues.inspModelClass);
    setInspManualClass(cleanValues.inspManualClass);
    setInspModelConf(cleanValues.inspModelConf);
    setErrors({});
  };
  const [inspectionRecord, setInspectionRecord] = React.useState(inspection);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Inspection, idProp)
        : inspection;
      setInspectionRecord(record);
    };
    queryData();
  }, [idProp, inspection]);
  React.useEffect(resetStateValues, [inspectionRecord]);
  const validations = {
    imageS3Url: [],
    inspModelTS: [],
    inspManualTS: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Inspection.copyOf(inspectionRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "AutomaticClassification")}
      {...rest}
    >
      <TextField
        label="Image s3 url"

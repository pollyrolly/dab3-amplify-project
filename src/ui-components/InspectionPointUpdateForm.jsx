...
export default function InspectionPointUpdateForm(props) {
  const {
    id: idProp,
    inspectionPoint,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = inspectionPointRecord
      ? { ...initialValues, ...inspectionPointRecord }
      : initialValues;
    setModel(cleanValues.model);
    setModelDeployed(cleanValues.modelDeployed);
    setInspModus(cleanValues.inspModus);
    setErrors({});
  };
  const [inspectionPointRecord, setInspectionPointRecord] =
    React.useState(inspectionPoint);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(InspectionPoint, idProp)
        : inspectionPoint;
      setInspectionPointRecord(record);
    };
    queryData();
  }, [idProp, inspectionPoint]);
  React.useEffect(resetStateValues, [inspectionPointRecord]);
  const validations = {
    model: [],
    modelDeployed: [],
    inspModus: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            InspectionPoint.copyOf(inspectionPointRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "InspectionPointUpdateForm")}
      {...rest}
    >
      <TextField
        label="Model"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || inspectionPoint)}
